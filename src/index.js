...
function shouldCompress(req, res) {
    ...
    return compression.filter(req, res); //decide if the response should be considered for compression. 
}

app.use(
    compression({
        level: 2, // 0 for no compression, set compression level from 1 to 9 (6 by default), 1 for fast and 9 for best
...
app.get('*', (req, res) => {
    ...
    const promises = routes
        .map(({ route }) => {
            return route.loadData ? route.loadData(store, id, selectedSearch) : null;
        })
        .map(promise => {
            if (promise) {
